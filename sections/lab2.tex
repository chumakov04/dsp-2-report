...
Basic comb filter with $r$ = 0.9999995 and $Lf$ = 9 uses FIR structure with $a = 1$ so only zeros, no poles. 9 zeros are placed near unit circle creating equally spaced notches across 0 to $\pi$.
...
Order $N$ = 4 butterworth lowpass with cutoff at $0.2\pi$, meaning that transfer function has 4 poles and 4 zeros. combFilter with $n = 3$ inserts 3 zeros between each coefficient. Lowpass filter shape is repeated 3 times across spectrum 0 to $\pi$. Phase compressed to $0.33\pi$, which is $1/3$ of original lowpass filter range between 0 and $\pi$, repeats 3 times. Poles and zeros are also copied 3 times at multiples of $0.33\pi$ frequency inside unit circle.
