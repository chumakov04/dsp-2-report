...
Figure~\ref{fig:34} shows difference signal in time domain between original $x(t)$ (or $s0$ as I used in code and plots) and recovered $e(k)$. As can be seen, most noticable difference is as already observed until 1 second, when FIR filter coefficients settle towards optimal values. However, most difference is actually until 0.3 seconds (not 1 second as I concluded from visual difference between original and recovered signal), after that coefficients converge and difference become minimal with small variance.
...
Only change is the input signal which is real one with sound.wav file that has 2 channels. Time domain spectrum clearly shows noisy disturbed signal overlapped with intereference noise signal (Figure~\ref{fig:306}).

\begin{figure}[H]
  \includegraphics[width=0.6\textwidth]{image_101.jpg}
  \caption{sound.wav time domain}\label{fig:306}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{sound.wav 2 channels and adaptive filtering output}\label{fig:36}
\end{figure}

Figure~\ref{fig:36} shows what task says. Channel 1 given with samples of disturbed signal and channel 2 samples of noise interference signal. 
...
Only change I made is adaptation rate $mu$ to be 0.005 instead of 0.001, which played more clear output signal, judged by hearable difference. Figure~\ref{fig:36} shows recovered signal that has overall pattern as with noise in time domain but output is very clear almost without any noise interference. 
