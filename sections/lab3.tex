...
Following theoretical part, error signal $e(k)$ is calculated as difference between disturbed signal $d(k)$ and FIR filtered output signal $y$ produced by convolution of $b_n$ coefficients with noise signal samples. Then new $b_n$ coefficients are updated using same lms update rule (\ref{eq:lms}) as in previous subsection.  So, nothing is modified in the adaptive filter routine, only channel 1 and channel 2 from sound.wav are correctly used as $d$ and $x$ inputs as task specifies.
...
subplot(3,1,3); plot(t, e); title('recovered signal');
sound(e, fa);
\end{matlab}
...
Figure~\ref{fig:37} shows that only coefficient playing role is $b_0$ converging towards 1, with other coefficients $b_1$, $b_2$, $b_3$, $b_4$ converging towards 0. This means that noise added to signal has transfer function close to $[1, 0, 0, 0, 0]$, equivalently noise is added directly without delay or filtering. Therefore adaptive filter only needs to scale noise signal by factor of 1 and subtract it.

\codeused
\begin{matlab}
figure;
plot(bb');
title('b coefficients over time');
xlabel('sample');
\end{matlab}